...
class VoteServiceTest {
    ...
    void getUserVoteTypeOnThread_Success() {
        // Arrange
        when(voteRepository.findByUserIdAndThreadId(1L, 1L))
    ...
    void getThreadVoteCount_Success() {
        // Arrange
        thread.setUpvoteCount(5);
        thread.setDownvoteCount(2);
        when(threadRepository.findById(1L)).thenReturn(Optional.of(thread));
        ...
        // Assert
        assertEquals(3, result); // 5 upvotes - 2 downvotes
    }
    ...
    void getThreadVoteCount_ThreadNotFound() {
        ...
        // Act & Assert
        assertThrows(EntityNotFoundException.class, () -> 
            voteService.getThreadVoteCount(1L));
    }
} 